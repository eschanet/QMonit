...
parser.add_argument(
    '--skipSubmit', action='store_true', help='do not upload to DB'
)
    ...
    for (panda_queue, prod_source, resource) in read_cursor:
        try:
            nickname = panda_resources[
                panda_queue]  #do the mapping to actual panda queue nicknames
        ...
        ddm_names = panda_queues.get(nickname, {}).get("astorages",
                                                       {}).get("read_lan", [])
        ...
        if len(datadisk_names) > 1:
            logger.warning(
                "Got more than one datadisk for: %s, %s" %
                (atlas_site, datadisk_names)
            )

...
def get_derived_quantities(distinct_sets, series, series_30d, pqs_mysql):
    ...
    for rs in distinct_sets.keys():
        ...
        if data is None:
            mysql_data[rs["panda_queue"]][rs["prod_source"]][rs["resource"]][
                rs["job_status"]
            ] = None
            mysql_data[rs["panda_queue"]][rs["prod_source"]][rs["resource"]][
                "tags"
...
def get_derived_quantities_for_keyset(rs, series, series_30d):
    logger.debug(
        "Queue: %s     Prod source: %s       Resource: %s      State: %s"
        % (rs["panda_queue"], rs["prod_source"], rs["resource"], rs["job_status"])
    )
    ...
    if len(filtered_points_30d) == 0:
        ...
        logger.debug(
            "Uhh, oh, got more than one point? This is weird! I will use the first one and hope this is what you meant to do."
        )
    ...
    data["avg7d"] = get_average_jobs(
        7, values_30d, columns_30d.index("jobs"), debug=args.debug, skipFirst=True
    )
    data["avg30d"] = get_average_jobs(
        30, values_30d, columns_30d.index("jobs"), skipFirst=True
    )
        ...
        for prod_source, resources in d.iteritems():
            for resource, job_states in resources.iteritems():
                ...
                add_point = """INSERT INTO jobs (panda_queue,prod_source, resource) VALUES ("{panda_queue}", "{prod_source}", "{resource}") ON DUPLICATE KEY UPDATE """.format(
                    panda_queue=panda_queue, prod_source=prod_source, resource=resource
                )

                for field, value in temp_data.iteritems():
                    add_point += """{field}={value}, """.format(
...
def run():
    ...
    data, missing_pqs = get_derived_quantities(
        rs_distinct_sets, series_24h, series_30d, pqs_mysql
    )
    ...
    for pq, prod_source, resource in missing_pqs:
        cursor.execute(
            'DELETE FROM jobs WHERE panda_queue = "{panda_queue}" AND resource = "{resource}" AND prod_source = "{prod_source}"'.format(
                panda_queue=pq, resource=resource, prod_source=prod_source
            )
        )
